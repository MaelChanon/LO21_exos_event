...
namespace TIME {
	/*! \class TimeException
			\brief Classe permettant de garer les exceptions des classes du namespace TIME 		
	*/
	class TimeException{
	public:
		//! Constructeur a partir d'une string
		TimeException(const std::string& m):info(m){}
		const std::string& GetInfo() const { return info; } //<! Retourne l'information stockae dans la classe
	private:
		std::string info;
	};
	...
	/*! \class Date
			...
			L'utilisation de cette classe nacessite des dates valides au sens commun du terme. 
			Daclenchement d'exception dans le cas contraire
	*/	
	class Date {
	public:
		//! Constructeur a partir d'un jour, mois, annae
		/*! \param j jour avec 1<=j<=31
			\param m mois avec 1<=m<=12
			\param a annae avec a>=0
			*/
		...
		unsigned int getAnnee() const { return annee; } //<! Retourne l'annae de la date
		...
		bool operator==(const Date& d) const; //<! d1==d2 retourne vrai si les deux dates sont agales
		bool operator<(const Date& d) const; //<! Compare deux dates dans le temps : d1<d2 retourne true si d1 est avant d2
		int operator-(const Date& d) const; //<! Retourne le nombre de jours saparant les deux dates
	...
	*/	
	...
	/*! \class Periode
			\brief Classe permettant de manipuler des periodes exprimaes en jours/mois/annaes	
			L'utilisation de cette classe nacessite des dates valides au sens commun du terme. 
			Daclenchement d'exception dans le cas contraire
	*/	
	class Periode{
	public :
		//! Constructeur a partir de jour/mois/annae
		/*! \param j nombre de jours avec 0<=j<=364
			\param m nombre de mois avec 0<=m<=11
			\param a nombre d'annaes 
			*/
	...
	*/	
	class Intervalle{
	public:
		//! Constructeur a partir de deux dates
		/*! \param d date de dabut de l'intervalle
		...
		Date getDebut() const { return debut; } //<! Retourne la date de dabut de l'intervalle
		Date getFin() const { return fin; } //<! Retourne la date de fin de l'intervalle
		int getDuree() const { return fin-debut; } //<! Retourne le nombre de jours s'acoulant entre le dabut et la fin de l'intervalle
